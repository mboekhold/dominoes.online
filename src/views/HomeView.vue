<template>
  <main class="lg:ml-20">
    <section class="p-2 lg:px-20 2xl:px-64 mx-auto text-white mt-10">
      <!-- Layout: Image and Hero Section -->
      <div class="flex flex-col-reverse lg:flex-row">
        <!-- Game Image -->
        <div class="w-full mt-10 mb-10 md:mb-0">
          <img src="@/assets/game_screenshot.png" alt="Dominoes online multiplayer game screenshot" loading="lazy" />
        </div>
        ...
        <div class="lg:ml-10 w-full lg:p-10 mb-20">
          ...
          <p class="mt-10 text-xl lg:text-2xl text-center">
      ...
      <!-- Leaderboard Section - Not SEO critical -->
      <section class="mt-10 mb-20" aria-hidden="true">
        ...
        <div v-else>
            ...
            <div class="relative w-full h-36 flex items-start p-2 lg:p-8 bg-night-dark-3 rounded-md">
              ...
              <div>
                <img v-if="leaderBoard[0].player" :src="getUserAvatar(leaderBoard[0].player)" alt="Player avatar"
                  loading="lazy" class="ml-5 w-20 h-20 rounded-md border-yellow-500 border-2" />
                <div v-else class="ml-5 w-12 h-12 lg:w-20 lg:h-20 rounded-md border-yellow-500 border-2"></div>
              </div>
              <div class="ml-5">
                ...
                <div class="flex items-center">
                  <img v-if="leaderBoard[0].player && leaderBoard[0].player.countries"
                    :src="leaderBoard[0].player.countries.flag_url" alt="Country flag" loading="lazy"
                    class="w-6 rounded-sm" />
                  <div class="text-xs ml-1">
                    {{ leaderBoard[0].player?.countries?.name || '---' }}
                  </div>
                </div>
                <div class="w-fit absolute lg:relative left-5 bottom-4 lg:left-0 lg:bottom-0">
                  <div class="flex items-center text-sm mt-2">
                    <div class="text-blue-400">
                      {{ leaderBoard[0].player ? getWinPercentage(leaderBoard[0].player) + '%' : '----' }}
                    </div>
                    <div class="text-xs ml-1 text-gray-500"> / </div>
                    <div class="ml-1">
                      {{ leaderBoard[0].player ? leaderBoard[0].player.games_played + ' games' : '----' }}
                    </div>
                  </div>
          ...
          <div class="mt-3 grid grid-cols-1 lg:grid-cols-4 gap-3">
            <div v-for="rank in leaderBoard.slice(1)" :key="rank.nr">
              <div class="bg-night-dark-3 rounded-md p-2 relative lg:h-32">
                <div class="flex items-center">
                  ...
                  <div>
                    <img v-if="rank.player" :src="getUserAvatar(rank.player)" alt="Player avatar" loading="lazy"
                      class="ml-5 w-8 h-8 lg:w-12 lg:h-12 rounded-md border-yellow-500 border-2" />
                    <div v-else class="ml-5 w-8 h-8 lg:w-12 lg:h-12 rounded-md border-yellow-500 border-2"></div>
                  </div>
                  <div class="ml-5">
                    <div>
                    ...
                    <div class="flex items-center text-xs">
                      <img v-if="rank.player?.countries" :src="rank.player.countries.flag_url" alt="Country flag"
                        loading="lazy" class="w-4 rounded-sm" />
                      <div class="text-xs ml-1">
                        {{ rank.player?.countries?.name || '---' }}
                      </div>
                    </div>
                  </div>
                  <div
                    ...
                    <div class="mx-auto w-fit relative">
                      <div class="flex items-center text-sm lg:mt-2">
                        <div class="text-blue-400">{{ rank.player ? getWinPercentage(rank.player) + '%' : '----' }}
                        </div>
                        <div class="text-xs ml-1 text-gray-500"> / </div>
...
export default {
  ...
  methods: {
    ...
    async loadLeaderBoard() {
      ...
      for (let i = 0; i < 5; i++) {
        this.leaderBoard.push({ nr: i + 1, player: false });
      }
      try {
  ...
  setup() {
    ...
    useHead({
      title: 'Play Dominoes Online | Multiplayer & Tournaments',
      meta: [
        {
          name: 'description',
          content: 'Join the top online Dominoes game. Play against others, compete in tournaments, and climb the leaderboard!'
        },
        {
          property: 'og:title',
          content: 'Play Dominoes Online'
        },
        {
          property: 'og:description',
          content: 'Compete in online Dominoes tournaments and see your name on the leaderboard.'
        },
        {
          property: 'og:image',
          content: '/images/dominoes-og-image.jpg'
        }
      ]
    });
  }
